...
function renderBox(
  ...
  choiceEntries.forEach(({ key, label }, idx) => {
    ...
    const check = selectedSet ? (selectedSet.has(idx) ? '[x]' : '[ ]') : '  ';
    const prefix = `${indicator} ${check} `;
    const text = key ? `${key}) ${label}` : String(label);
    const wrapped = wrapWithLead(text, prefix, targetWidth || termLimit);
  ...
  const formatLine = (line, isSelected) => {
    const padded = line.padEnd(contentWidth, ' ');
    const rendered = isSelected ? highlightFn(padded) : padded;
